    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if payment.get('state') == 0:
                order_heleket_payment_id = payment.get('result', {}).get('uuid')
...
def heleket_webhook(request):
    ...
    try:
        ...
        payload = request.bode.decode('utf-8')
        encdoded_payload = base64.b64encode(payload.encode('utf-8')).decode('utf-8')
        expected = hashlib.md5((encdoded_payload + HELEKET_SECRET_KEY).encode('utf-8')).hexdigest()
        ...
        if not order_id:
            return HttpResponseBadRequest('Missing_order_id')
    ...
    if order_id:
        try:
            ...
            if order.status == 'completed':
                ...
                if request.headers.get('HX-Request'):
                    return TemplateResponse(request, 'payment/leket_success_content.html', context)
            ...
            context = {'order': order}
            if request.headers.get('HX-Request'):
                return TemplateResponse(request, 'payment/heleket_pending_content.html', request)
